class FreeConsultationsController < ApplicationController
  ...
  def create
    @free_consultation_booking =
      FreeConsultationBooking.new(free_consultation_booking_params)

    @free_consultation_booking.user = current_user

    # 🔒 BLOCCO SLOT GIÀ PRENOTATO
    if FreeConsultationBooking.exists?(
         scheduled_at: @free_consultation_booking.scheduled_at
       )
    ...
    if @free_consultation_booking.save
      ...
      AvailabilitySlot
        .find_by(scheduled_at: @free_consultation_booking.scheduled_at)
        &.update(active: false)
